...
- **No inline SQL strings** in `.rs` files; use `src-tauri/sql/<area>/Xxx.sql` with `include_str!()`.
- **No inline LLM/AI prompts** in `.rs` files; use `src-tauri/prompts/<area>/Xxx.prompt` with `include_str!()`.
